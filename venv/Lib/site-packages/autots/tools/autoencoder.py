...
class VAEAnomalyDetector:
    ...
    def _compute_loss(self, recon_x, x, mean, logvar):
        ...
        if self.loss_function == 'mse':
            ...
            recon_loss = F.mse_loss(
                torch.log(recon_x + 1e-8), torch.log(x + 1e-8), reduction='sum'
            )
    ...
    def fit(self, X):
        ...
        # Create model
        self.model = VAE(input_dim, self.latent_dim, self.depth, self.dropout_rate).to(
            self.device
        )
    ...
    def predict(self, X):
        ...
        if self.loss_function == 'lmse':
            reconstruction_error = np.mean(
                (np.log(X_scaled + 1e-8) - np.log(recon_x + 1e-8)) ** 2, axis=1
            )
        else:
