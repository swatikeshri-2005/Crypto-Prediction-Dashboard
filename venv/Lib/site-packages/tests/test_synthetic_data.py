...
class TestSyntheticDataGeneration(unittest.TestCase):
    ...
    def test_basic_generation(self):
        ...
        self.assertIsInstance(
            data.index, pd.DatetimeIndex, "Index should be DatetimeIndex"
        )
    ...
    def test_business_day_series(self):
        ...
        self.assertTrue(
            weekend_values.isna().all(), "series_0 should have NaN on all weekends"
        )
    ...
    def test_trend_changepoints(self):
        ...
        if len(series_0_cp) > 0:
            ...
            self.assertIsInstance(
                old_slope, (int, float, str), "Slope should be numeric or label"
            )
    ...
    def test_level_shifts(self):
        ...
        self.assertEqual(
            len(series_4_shifts), 0, "series_4 should have no level shifts"
        )

    def test_anomalies(self):
        """Test anomaly generation and magnitude."""
        gen = generate_synthetic_daily_data(
            n_days=730, n_series=5, random_seed=42, anomaly_freq=0.1
        )
    ...
    def test_seasonality(self):
        ...
        self.assertGreaterEqual(
            len(season_cp_2), 2, "series_2 should have seasonality markers"
        )

        # series_3 should have seasonality changepoints
        season_cp_3 = gen.get_seasonality_changepoints('series_3')
        self.assertGreaterEqual(
            len(season_cp_3), 1, "series_3 should have seasonality changepoints"
        )
    ...
    def test_holidays(self):
        ...
        christmas_dates = [
            d for d in holidays_0.keys() if d.month == 12 and d.day == 25
        ]
        self.assertGreaterEqual(
            len(christmas_dates), 1, "Should have at least one Christmas"
        )

        # Check Lunar holidays (series_5)
        holidays_5 = gen.get_holiday_impacts('series_5')
        self.assertGreater(
            len(holidays_5), 0, "series_5 should have Lunar holiday impacts"
        )
        ...
        self.assertGreater(
            len(holidays_6), 0, "series_6 should have Ramadan holiday impacts"
        )
        ...
        self.assertGreater(
            len(dom_keys), 0, "Should generate random day-of-month holidays"
        )
        self.assertGreater(
            len(wkdom_keys), 0, "Should generate random weekday-of-month holidays"
        )
        ...
        for dom_key in dom_keys:
            ...
            for series_dates in all_holiday_dates:
                if any(
                    date.month == month and date.day == day for date in series_dates
                ):
            ...
            if dom_match_found:
                break
        self.assertTrue(
            dom_match_found, "Random dom_* holiday should appear in generated dates"
        )
        ...
        for wkdom_key in wkdom_keys:
            ...
            if wkdom_match_found:
                break
        self.assertTrue(
            wkdom_match_found, "Random wkdom_* holiday should appear in generated dates"
        )

    def test_regressors(self):
        """Test regressor generation."""
        gen = generate_synthetic_daily_data(
            n_days=365, n_series=5, random_seed=42, include_regressors=True
        )
        ...
        self.assertIn(
            'promotion', regressors.columns, "Should have promotion regressor"
        )
        self.assertIn(
            'temperature', regressors.columns, "Should have temperature regressor"
        )
        self.assertIn(
            'precipitation', regressors.columns, "Should have precipitation regressor"
        )
    ...
    def test_noise_changepoints(self):
        ...
        self.assertIsInstance(
            noise_cp, list, "Should return list of noise changepoints"
        )
    ...
    def test_variance_regimes(self):
        ...
        self.assertIsInstance(
            noise_cp_7, list, "series_7 should have variance regime info"
        )
    ...
    def test_autocorrelated_noise(self):
        ...
        self.assertGreater(
            lag1_corr, 0.5, "AR noise should have strong positive autocorrelation"
        )
        ...
        self.assertLess(
            var_ratio, 1.5, "Variance should be relatively stable (stationary)"
        )
    ...
    def test_multiplicative_seasonality(self):
        ...
        self.assertIn(
            'phi', ar_params, "Should use AR noise for multiplicative seasonality"
        )
    ...
    def test_component_decomposition(self):
        ...
        self.assertTrue(
            np.allclose(component_sum, actual_series, equal_nan=True),
    ...
    def test_saturating_trend(self):
        ...
        has_saturation_marker = any(
            'saturation' in t or 'quadratic' in t for t in cp_types
        )

        self.assertTrue(
            has_saturation_marker, "series_1 should have saturation markers"
        )
    ...
    def test_plot_method(self):
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode='w', delete=False, suffix='.png'
            ) as f:
                temp_file = f.name
            ...
            try:
                ...
                self.assertTrue(
                    os.path.exists(temp_file), "Plot file should be created"
                )
    ...
    def test_machine_summary_structure(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                self.assertIn(
                    'series', summary, "Summary should include per-series entries"
                )
                self.assertEqual(
                    summary['meta']['n_series'],
                    3,
                    "Metadata should reflect series count",
                )
                self.assertEqual(
                    len(summary['series']), 3, "Should include all series details"
                )
                ...
                self.assertTrue(
                    expected_keys.issubset(first_series.keys()),
                    "Missing expected series metadata",
                )

                events = first_series.get('events')
                self.assertIsInstance(
                    events, dict, "Events should be present when include_events=True"
                )
                self.assertIn(
                    'holidays', events, "Holiday details should be included in events"
                )
                ...
                self.assertEqual(
                    len(trend_list),
                    expected_length,
                    "Trend list should respect max limit",
                )

                filtered = gen.machine_summary(
                    series_name=first_series['name'], max_events_per_type=1
                )
                self.assertEqual(
                    len(filtered['series']),
                    1,
                    "Filtered summary should only include requested series",
                )
                self.assertEqual(
                    filtered['series'][0]['name'],
                    first_series['name'],
                    "Filtered summary should match series",
                )

                json_payload = gen.machine_summary(
                    series_name=first_series['name'],
                    max_events_per_type=1,
                    as_json=True,
                )
                parsed = json.loads(json_payload)
                self.assertIsInstance(
                    parsed, dict, "JSON payload should parse into dict"
                )
                self.assertEqual(
                    parsed['series'][0]['name'],
                    first_series['name'],
                    "JSON payload should contain requested series",
                )
        finally:
    ...
    def test_slope_changes_are_meaningful(self):
        ...
        for series_name in all_changepoints.keys():
            ...
            min_threshold = (
                0.008 * scale * 0.5
            )  # Allow subtle changes at 50% of base threshold
    ...
    def test_shared_events(self):
        ...
        shared_anomaly_found = any(
            len(series_list) > 1 for series_list in anomaly_dates.values()
        )
        ...
        shared_shift_found = any(
            len(series_list) > 1 for series_list in shift_dates.values()
        )
        self.assertTrue(
            shared_shift_found, "Should find at least one shared level shift"
        )
    ...
    def test_anomaly_types_parameter(self):
        ...
        if len(anomaly_types_found2) > 0:
            self.assertTrue(
                anomaly_types_found2.issubset({'impulse_decay', 'linear_decay'}),
        ...
        with self.assertRaises(ValueError):
            generate_synthetic_daily_data(
                n_days=365, n_series=3, random_seed=42, anomaly_types=['invalid_type']
            )
    ...
    def test_tune_to_data(self):
        ...
        params_changed = sum(
            abs(new_params[k] - orig_params[k]) > 0.01 for k in orig_params.keys()
        )
        self.assertGreater(
            params_changed, 0, "At least some parameters should change during tuning"
        )
    ...
    def test_disable_holiday_splash_parameter(self):
        ...
        gen_no_splash = generate_synthetic_daily_data(
            n_days=730, n_series=10, random_seed=42, disable_holiday_splash=True
        )
        ...
        for holiday_name, config in holiday_config.items():
            self.assertFalse(
                config['has_splash'], f"{holiday_name} should have splash disabled"
            )
            self.assertFalse(
                config['has_bridge'], f"{holiday_name} should have bridge disabled"
            )
        ...
        gen_with_splash = generate_synthetic_daily_data(
            n_days=730, n_series=10, random_seed=42, disable_holiday_splash=False
        )
        ...
        some_enabled = any(
            config['has_splash'] or config['has_bridge']
