...
class NVARTest(unittest.TestCase):
    ...
    def test_nvar_basic_fit_predict(self):
        ...
        model = NVAR(k=1, ridge_param=2.5e-6, warmup_pts=1, seed_pts=1, batch_size=5)
    ...
    def test_nvar_with_prediction_object(self):
        ...
        model = NVAR(k=1, prediction_interval=0.9, seed_pts=3)
        ...
        for batch_size in [2, 5, 10]:
            with self.subTest(batch_size=batch_size):
                ...
                forecast = model.predict(
                    forecast_length=forecast_length, just_point_forecast=True
                )
    ...
    def test_nvar_realistic_data(self):
        ...
        self.assertFalse(
            forecast.isna().any().any(), "Forecast contains NaN with clean data"
        )
    ...
    def test_predict_reservoir_seed_pts_interval_corruption(self):
        ...
        self.assertTrue(
            np.all(pred_upper >= pred_lower), "Upper bound should be >= lower bound"
        )

        # Prediction should generally be within bounds
        # (allowing some tolerance for numerical issues)
        within_bounds = np.logical_and(
            pred >= pred_lower - 1e-10, pred <= pred_upper + 1e-10
        )
        self.assertTrue(
            np.all(within_bounds), "Prediction should be within interval bounds"
        )
        ...
        self.assertLess(
            max_relative_change, 10.0, "Interval width shouldn't have extreme jumps"
        )
        ...
        for batch_method in ['med_sorted', 'std_sorted', 'max_sorted']:
            with self.subTest(batch_method=batch_method):
                ...
                for _ in range(5):
                    ...
                    model.fit(df.copy())
                    forecast = model.predict(
                        forecast_length=5, just_point_forecast=True
                    )
...
class NVAREarlyStoppingTest(unittest.TestCase):
    ...
    def test_early_stopping_prevents_inf(self):
        ...
        self.assertFalse(
            np.isinf(forecast.values).any(), "Early stopping should prevent Inf values"
        )
    ...
    def test_early_stopping_with_large_values(self):
        ...
        self.assertLess(
            forecast_scale, train_scale * 100, "Forecast scale unreasonably large"
        )
    ...
    def test_early_stopping_with_intervals(self):
        ...
        model = NVAR(
            k=1, ridge_param=1e-7, batch_size=2, seed_pts=5, prediction_interval=0.9
        )
        model.fit(df)
