        ...
        for key, constraint in constraint_types.items():
            with self.subTest(i=key):
                ...
                if key in ["last_value"]:
                    self.assertTrue(
                        prediction.forecast.iloc[0, :].max() == df.iloc[-1, :].max()
                    )
                # test for nulls
    ...
    def test_adjustments(self):
        ...
        with self.subTest(i="percentage_constant"):
            ...
            pd.testing.assert_frame_equal(
                pred.lower_forecast, expected_lower, rtol=1e-10
            )
        ...
        with self.subTest(i="percentage_gradient"):
            ...
            self.assertTrue(
                (pred.forecast.iloc[0, :] - orig_forecast.iloc[0, :]).abs().max()
                < 1e-10
            )
            expected_last = orig_forecast.iloc[-1, :] * 1.2
            pd.testing.assert_series_equal(
                pred.forecast.iloc[-1, :], expected_last, rtol=1e-10
            )
        ...
        with self.subTest(i="align_last_value"):
            ...
            for col in df.columns:
                ...
                self.assertAlmostEqual(
                    first_forecast, last_train, delta=abs(last_train * 0.01) + 0.1
                )
        ...
        with self.subTest(i="date_range"):
            ...
            pd.testing.assert_frame_equal(
                pred.forecast.iloc[:mid_idx, :], orig_forecast.iloc[:mid_idx, :]
            )
            # Second half should be adjusted
            expected_second_half = orig_forecast.iloc[mid_idx:, :] + 5.0
            pd.testing.assert_frame_equal(
                pred.forecast.iloc[mid_idx:, :], expected_second_half
            )
        ...
        with self.subTest(i="percentage_gradient_with_date_range"):
            ...
            pd.testing.assert_frame_equal(
                pred.forecast.iloc[:quarter_idx, :], orig_forecast.iloc[:quarter_idx, :]
            )
            ...
            pd.testing.assert_series_equal(
                pred.forecast.iloc[quarter_idx, :], expected_start, rtol=1e-10
            )

            # At end of range should be increased by ~10%
            expected_end = orig_forecast.iloc[three_quarter_idx, :] * 1.1
            pd.testing.assert_series_equal(
                pred.forecast.iloc[three_quarter_idx, :], expected_end, rtol=1e-10
            )
