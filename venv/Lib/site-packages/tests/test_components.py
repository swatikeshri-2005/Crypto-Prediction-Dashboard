...
class TestBasicLinearModelComponents(unittest.TestCase):
        ...
        cls.df = pd.DataFrame(
            {
                ...
                + np.sin(np.arange(100) * 2 * np.pi / 7) * 5,
                'series2': np.cumsum(np.random.randn(100))
                + np.cos(np.arange(100) * 2 * np.pi / 7) * 3,
            },
            index=cls.dates,
        )
    ...
    def test_component_types(self):
        ...
        component_types = (
            prediction.components.columns.get_level_values(1).unique().tolist()
        )
    ...
    def test_components_sum_to_forecast(self):
        ...
        model = BasicLinearModel(
            seasonal_period='weekly', trend_phi=0.98, prediction_interval=0.9
        )
        model.fit(self.df)
        prediction = model.predict(forecast_length=14)
        ...
        self.assertLess(
            diff, 1e-10, f"Components don't sum to forecast. Max difference: {diff}"
        )
    ...
    def test_components_with_regressors(self):
        ...
        prediction = model.predict(
            forecast_length=14, future_regressor=future_regressor
        )
    ...
    def test_components_without_trend_phi(self):
        ...
        model = BasicLinearModel(
            seasonal_period='weekly', trend_phi=None, prediction_interval=0.9
        )
        model.fit(self.df)
        prediction = model.predict(forecast_length=14)
    ...
    def test_forecast_not_reconstructed(self):
        ...
        phi_series = pd.Series([model.trend_phi] * req_len, index=test_index[1:]).pow(
            range(req_len)
        )
        ...
        self.assertLess(
            diff, 1e-10, "Forecast was not calculated using original method"
        )
