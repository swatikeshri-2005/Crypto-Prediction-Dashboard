...
class TestARDL(unittest.TestCase):
    ...
    def test_ardl_list_regression_feature_combination(self):
        ...
        model_fourier = ARDL(
            regression_type='common_fourier_rw', lags=1, order=1, verbose=0
        )
        model_fourier.fit(self.data_single)

        model_holiday = ARDL(
            regression_type='holiday', lags=1, order=0, holiday_country='US', verbose=0
        )
        model_holiday.fit(self.data_single)

        # Combined should have sum of individual feature counts
        expected_cols = (
            model_fourier.regressor_train.shape[1]
    ...
    def test_ardl_get_new_params_can_generate_list(self):
        ...
        self.assertTrue(
            found_list,
            "get_new_params should occasionally generate list regression types",
        )
    ...
    def test_ardl_single_regression_type(self):
        ...
        # Test None
        model_none = ARDL(regression_type=None, lags=1, order=0, verbose=0)
