...
class TestWindowFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.df_univariate = pd.DataFrame(
            {'series1': np.random.randn(100).cumsum() + 100}, index=dates
        )
        ...
        self.future_regressor = pd.DataFrame(
            {'regressor1': np.random.randn(100), 'regressor2': np.random.randn(100)},
            index=dates,
        )
    ...
    def test_last_window_basic(self):
        ...
        # Test univariate
        result = last_window(self.df_univariate, window_size=5, input_dim='univariate')
        ...
        # Test multivariate
        result = last_window(
            self.df_multivariate, window_size=5, input_dim='multivariate'
        )
    ...
    def test_last_window_options(self):
        ...
        X, Y = window_maker(small_df, window_size=5, forecast_length=2)
    ...
    def test_data_integrity_and_edge_cases(self):
        ...
        X, Y = window_maker(
            self.df_univariate, window_size=3, forecast_length=1, max_windows=None
        )
        ...
        np.testing.assert_array_almost_equal(
            first_window, expected_first_window, decimal=5
        )

        # Test last_window edge case
        result_large = last_window(
            self.df_univariate.head(3), window_size=5, input_dim='univariate'
        )
    ...
    def test_random_seed_consistency(self):
        ...
        X1, Y1 = window_maker(
            self.df_multivariate, window_size=5, max_windows=50, random_seed=42
        )
    ...
    def test_output_types(self):
        ...
        X, Y = window_maker(self.df_univariate, window_size=5, forecast_length=1)
