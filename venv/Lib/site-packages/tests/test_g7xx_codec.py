...
class TestG726AdpcmFilter(unittest.TestCase):
    ...
    def test_blend_parameter(self):
        ...
        filtered_low_blend = g726_adpcm_filter(
            data, blend=0.0, use_adaptive_predictor=False
        )
        filtered_high_blend = g726_adpcm_filter(
            data, blend=0.8, use_adaptive_predictor=False
        )
    ...
    def test_adaptive_predictor(self):
        ...
        t = np.arange(500)
        signal = (
            100 + 0.1 * t + 10 * np.sin(2 * np.pi * t / 7) + np.random.randn(500) * 2
        )
        ...
        # Legacy EMA mode
        filtered_legacy = g726_adpcm_filter(df, use_adaptive_predictor=False)
...
class TestG726FilterTransformer(unittest.TestCase):
    ...
    def test_smoothing_effect(self):
        ...
        transformer = G726Filter(
            use_adaptive_predictor=False, quant_bits=3, dynamic_range=2.0
        )
        transformed = transformer.fit_transform(df)
        ...
        correlation = np.corrcoef(transformed['series1'].values, df['series1'].values)[
            0, 1
        ]
