            ...
            with self.subTest(method=method):
                for test_name, (date_str, expected_month) in test_cases.items():
                    with self.subTest(test_name=test_name):
                        ...
                        if (
                            method == "simple"
                            and test_name in simple_method_known_issues
                        ):
                            self.skipTest(
                                f"Known limitation: {method} method has timing precision issues for {test_name}"
                            )

                        date = pd.to_datetime([date_str])
                        result = gregorian_to_hindu(date, method=method)
                        self.assertEqual(
                            result.iloc[0]['hindu_month_name'], expected_month
                        )
        ...
        for date_str, description in known_ramadan_starts.items():
            with self.subTest(description=description):
                ...
                self.assertIn(
                    result.iloc[0]['day'],
                    [1, 2],
                    "Failed day check - should be day 1 or 2",
                )
...
class TestHolidayFlag(unittest.TestCase):
    def test_holiday_flag(self):
        ...
        flag_1 = holiday_flag(
            input_dates, country="US", encode_holiday_type=False, holidays_subdiv="PR"
        )
        ...
        flag_2 = holiday_flag(
            input_dates, country="US", encode_holiday_type=True, holidays_subdiv=None
        )
...
class TestSeasonal(unittest.TestCase):
    def test_date_part(self):
        ...
        date_part_df = date_part(
            input_dates,
            method=['simple_binarized', 365.25, 'quarter'],
            set_index=True,
            holiday_country=["US"],
        ...
        self.assertEqual(
            np.sum(date_part_df.isnull().to_numpy()), 0, msg="date part generating NaN"
        )
